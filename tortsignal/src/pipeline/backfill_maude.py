"""
MAUDE Timeseries Backfill Pipeline

Populates historical timeseries data for MAUDE (device adverse events) by quarter.
This enables trend analysis: slope, acceleration, consecutive increases, etc.

Run with: python -m src.pipeline.backfill_maude --start-year 2022 --end-year 2025
"""
...
def fetch_maude_quarter_data(
    connector: MAUDEConnector,
    start: datetime,
    end: datetime,
    top_n: int = 500,
    max_retries: int = 3,
    ...
    """
    Fetch top devices for a specific quarter with retry logic.

    Args:
        connector: MAUDE connector instance
        start: Quarter start date
        end: Quarter end date
        top_n: Number of top devices to fetch
        max_retries: Maximum retry attempts for 500 errors

    Returns:
        List of device data with counts
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"  Fetched {len(results)} devices for {start.strftime('%Y-Q%q')}")
                return [
                    {
                        "device_name": item["term"],
                        "count": item["count"],
                    }
                    for item in results
                ]
...
def backfill_maude_timeseries(
    ...
    """
    Backfill MAUDE timeseries data by quarter.

    Args:
        start_year: First year to backfill
        end_year: Last year to backfill (inclusive)
        top_n: Number of top devices per quarter
    ...
    """
    logger.info("=" * 70)
    logger.info("MAUDE TIMESERIES BACKFILL")
    logger.info(f"Period: {start_year} - {end_year}, Top {top_n} devices per quarter")
    ...
    try:
        ...
        for year in range(start_year, end_year + 1):
            for quarter in range(1, 5):
                ...
                devices = fetch_maude_quarter_data(connector, quarter_start, quarter_end, top_n)
                    ...
                    for device_data in devices:
                        try:
                            upsert_timeseries_record(
                                cur,
                                product_name=device_data["device_name"],
                                source="maude",
                                period_start=quarter_start,
                                period_end=quarter_end,
                                report_count=device_data["count"],
                            )
                        except Exception as e:
                            logger.error(f"Failed to insert {device_data['device_name']}: {e}")
                            results["errors"] += 1

                results["quarters_processed"] += 1
                results["total_records"] += len(devices)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Backfill MAUDE timeseries data")
    ...
    parser.add_argument("--top-n", type=int, default=500, help="Top N devices per quarter")
    parser.add_argument("--dry-run", action="store_true", help="Don't write to database")
    args = parser.parse_args()

    try:
        results = backfill_maude_timeseries(
